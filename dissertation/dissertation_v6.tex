...
\begin{itemize}
  ...
  \item A systematic empirical analysis of how evaluation methodology---specifically the choice of data splitting strategy---inflates both detection and classification performance in IoT intrusion detection. Host-disjoint evaluation reduces XGBoost's multiclass macro-F1 from 1.000 to 0.736 and binary ROC-AUC from 1.000 to 0.842, even when identifier features are excluded, demonstrating that host-level flow correlation is a previously underreported source of evaluation leakage.
\end{itemize}
...
Split-strategy leakage analysis & Overoptimistic benchmarks (§2.8.1) & Macro-F1: 1.000 $\to$ 0.736 (Table~\ref{tab:host-multiclass}) \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:host-multiclass} presents the two-stage pipeline results under host-disjoint splitting. Stage 1 detects attacks (binary); Stage 2 classifies the attack type among the three overlapping categories.

\begin{table}[H]
\centering
\caption{Host-disjoint multiclass attack-type classification (behaviour-only, 211,043 rows). Macro-F1 averages across the three overlapping attack types: Scanning, Ransomware, and MitM.}
\label{tab:host-multiclass}
\begin{tabular}{lccccccc}
\toprule
\textbf{Model} & \textbf{Binary} & \textbf{Type} & \textbf{Type} & \textbf{Scanning} & \textbf{Ransom.} & \textbf{MitM} & \textbf{Det.} \\
& \textbf{ROC-AUC} & \textbf{Macro-F1} & \textbf{Wtd-F1} & \textbf{F1} & \textbf{F1} & \textbf{F1} & \textbf{Frac.} \\
\midrule
XGBoost & 0.842 & \textbf{0.736} & 0.998 & 0.999 & 0.933 & 0.276 & 1.000 \\
Random Forest & 0.976 & 0.715 & 0.997 & 0.998 & 1.000 & 0.146 & 1.000 \\
LightGBM & 0.993 & 0.361 & 0.998 & 0.999 & 0.000 & 0.083 & 0.999 \\
LogReg & 0.807 & 0.499 & 0.997 & 0.997 & 0.000 & 0.000 & 0.998 \\
\bottomrule
\end{tabular}
\end{table}

The results reveal three findings that are invisible under stratified splitting. First, \textbf{multiclass macro-F1 drops from 1.000 (stratified) to 0.736 (host-disjoint) for the best model} (XGBoost), a 26.4 percentage point decrease. This demonstrates that the near-perfect stratified results reflect host-level memorisation rather than generalisable attack-type discrimination.

Second, \textbf{rare attack types are extremely difficult under host-disjoint evaluation}. MitM achieves a maximum F1 of only 0.276 (XGBoost) with just 4 test samples, and Ransomware F1 ranges from 0.000 (LogReg, LightGBM) to 1.000 (Random Forest) with only 7 test samples. These scores are highly unstable due to the tiny evaluation sets---a single misclassification changes MitM F1 by approximately 0.25.

Third, \textbf{the model ranking changes under host-disjoint evaluation}. Under stratified splitting, XGBoost and LightGBM both achieve multiclass F1 of 1.000. Under host-disjoint splitting, XGBoost (macro-F1 = 0.736) substantially outperforms LightGBM (macro-F1 = 0.361) because LightGBM fails entirely on Ransomware and MitM. Random Forest achieves the only perfect per-class F1 (Ransomware = 1.000), suggesting that its ensemble diversity helps on rare categories despite its poor explanation stability.
...
\begin{enumerate}
  \item \textbf{Near-perfect stratified detection masks substantial host-level leakage.} All models achieve PR-AUC and ROC-AUC of 1.000 under stratified splitting, but host-disjoint evaluation reduces XGBoost binary ROC-AUC to 0.842 and multiclass macro-F1 to 0.736. This 26.4 percentage point multiclass gap demonstrates that published near-perfect results on TON\_IoT are inflated by host-level flow correlation, even when identifier features are excluded.
...
\addlinespace
O2: Define feature regimes isolating leakage & Achieved & Regime A vs B comparison (Table~\ref{tab:leakage}); host-disjoint split reduces macro-F1 from 1.000 to 0.736 (Table~\ref{tab:host-multiclass}) \\
\addlinespace
