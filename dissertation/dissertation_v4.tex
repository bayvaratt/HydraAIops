...
\begin{table}[H]
\centering
\caption{CNN-LSTM detection results (behaviour-only regime, 211,043 rows).}
\label{tab:cnnlstm}
\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
Binary PR-AUC & 0.9998 \\
Binary ROC-AUC & 0.9995 \\
Binary F1 & 0.9970 \\
Macro-F1 & 0.9936 \\
Score range & [0.0003, 0.9999] \\
Training time & 1,756s (29.3 min) \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Comprehensiveness & $-0.187$ \\
Sufficiency & $-0.013$ \\
$k_{90}$ & 0.197 \\
Gini coefficient & 0.852 \\
RMA@5 (DoS) & 0.40 \\
IG timing & 3.5 ms/sample \\
\bottomrule
\end{tabular}
\end{table}

The CNN-LSTM's IG attributions differ qualitatively from the tree models' SHAP attributions. The $k_{90}$ of 0.197 indicates that approximately 20\% of features are needed to account for 90\% of attribution mass, compared to 0.2\% for XGBoost---reflecting the CNN-LSTM's more distributed feature usage. The Gini coefficient of 0.852 (vs.\ 0.998 for XGBoost) confirms this: the CNN-LSTM spreads attribution across more features, producing explanations that are less parsimonious but potentially more comprehensive. The top-5 IG features---\texttt{src\_ip\_bytes}, \texttt{dst\_ip\_bytes}, \texttt{dst\_bytes}, \texttt{dns\_qtype}, and \texttt{dst\_pkts}---are plausible network behaviour indicators, with RMA@5 $= 0.40$ for DoS attacks. The IG timing of 3.5~ms/sample is slower than TreeSHAP (0.04~ms) but remains suitable for near-real-time deployment.
