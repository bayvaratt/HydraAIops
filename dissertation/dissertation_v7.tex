...
Split-strategy leakage analysis & Overoptimistic benchmarks (§2.8.1) & Macro-F1: 1.000 $\to$ 0.742 (Table~\ref{tab:host-multiclass}) \\
\bottomrule
\end{tabular}
\end{table}
...
A test suite of 91 automated tests covers the critical components of the pipeline: data splitting invariants (non-empty partitions, disjointness, class presence), metric computation (PR-AUC, ROC-AUC, DeLong test), baseline sanity checks, model building and prediction, and XAI evaluation functions. Tests are executed before each experimental run to verify pipeline integrity.

A continuous integration workflow (GitHub Actions) is configured to run the full test suite on every push and pull request, ensuring that code changes do not break existing functionality.

\subsection{Project Timeline}
\label{sec:timeline}

The project was conducted over approximately 8 months (October 2025 to May 2026). The first phase (October--December) focused on literature review, dataset acquisition, and the design of the two-stage pipeline and five-criterion XAI framework. The second phase (January--March) implemented the tabular model pipeline, explainability methods, and the five-criterion evaluation module, with iterative testing through 91 automated tests. The third phase (March--April) conducted the full experimental evaluation, including three-seed runs, cross-dataset generalisation, and leakage interaction analysis. The final phase (April--May) focused on the deep learning extensions (CNN-LSTM, GNN), analysis, and writing.
...
The near-perfect stratified-split results warrant scrutiny. To quantify the contribution of the splitting strategy to reported performance, Table~\ref{tab:split-impact} compares XGBoost and CNN-LSTM under stratified versus host-disjoint splitting on the full deduplicated dataset (190,474 rows after removing 9.7\% exact duplicates).

\begin{table}[H]
\centering
\caption{Impact of splitting strategy on detection performance (behaviour-only, deduplicated TON\_IoT). Host split ensures no source IP appears in more than one partition.}
...
\textbf{The stratified split inflates XGBoost's ROC-AUC from 0.854 to 1.000}---a 14.6 percentage point gap---even after deduplication and identifier removal. This inflation occurs because stratified splitting allows flows from the same source host to appear in both training and test partitions. Flows from the same host during the same attack campaign share near-identical behavioural signatures (duration, byte counts, packet counts), enabling the model to achieve perfect discrimination by recognising host-level traffic patterns rather than generalisable attack behaviours.

Feature importance analysis under the host split confirms this interpretation: \texttt{conn\_state\_S0} alone accounts for 54\% of XGBoost's total feature importance, followed by \texttt{proto\_tcp} (20\%) and \texttt{conn\_state\_SF} (10\%). The \texttt{conn\_state} feature is highly predictive because certain connection states (e.g., \texttt{REJ} with 100\% attack prevalence, \texttt{S1} with 99.9\%) are near-deterministic indicators of attack traffic in the TON\_IoT testbed.

The host-split ROC-AUC of 0.854 (XGBoost) represents a more realistic estimate of deployment performance against previously unseen devices. LogReg's host-split ROC-AUC of 0.538---barely above random---demonstrates that the linear separability observed under stratified splitting does not transfer to unseen hosts, as host-specific flow distributions differ substantially.

These results are reported alongside the stratified results throughout this chapter. The stratified results remain valuable for XAI comparison (where models are compared on the same data partition) but should not be interpreted as deployment-realistic detection estimates.

\subsection{Host-Disjoint Multiclass Attack-Type Classification}
\label{sec:host-multiclass}

The host-disjoint split has a critical implication for multiclass evaluation: because attack types are unevenly distributed across hosts, a host-disjoint split can leave entire attack categories absent from the test partition. In this evaluation, the host split produces a test set containing only three of the nine attack types: Scanning (7,855 flows), Ransomware (7 flows), and Man-in-the-Middle (4 flows). Five attack types---Backdoor, DDoS, DoS, Injection, and Password---appear exclusively in training hosts, while XSS appears only in test hosts. This distribution reflects the real-world challenge that specific attacks may target specific devices; a deployment-realistic evaluation must contend with this heterogeneity.

Table~\ref{tab:host-multiclass} presents the multiclass results under host-disjoint splitting on a 20,000-row balanced test subsample drawn to ensure all eight attack types are represented. Test sample sizes are: Backdoor = 50, DDoS = 728, DoS = 287, Injection = 2,657, MitM = 4, Password = 2,246, Ransomware = 2,745, and Scanning = 2,482.

\begin{table}[H]
\centering
\caption{Host-disjoint multiclass attack-type classification (behaviour-only, 20,000-row balanced test subsample). Per-class F1 scores for all eight attack types present in the test partition.}
...
The results reveal four findings that are invisible under stratified splitting. First, \textbf{multiclass macro-F1 drops from 1.000 (stratified) to 0.742 (host-disjoint) for the best model} (Random Forest), a 25.8 percentage point decrease. This demonstrates that the near-perfect stratified results reflect host-level memorisation rather than generalisable attack-type discrimination.

Second, \textbf{DoS is the hardest attack type under host-disjoint evaluation}, with a maximum F1 of only 0.080 (XGBoost). DoS flows are systematically confused with DDoS due to their similar volumetric signatures at the flow level; distinguishing them requires session-level or source-count features that are lost in per-flow aggregation. MitM remains unstable (maximum F1 = 0.191) with only 4 test samples, where a single misclassification changes F1 by approximately 0.25.

Third, \textbf{six of eight attack types achieve F1 $> 0.82$ for the best model}. Ransomware (F1 = 0.994), Password (F1 = 0.988), Injection (F1 = 0.982), DDoS (F1 = 0.955), Scanning (F1 = 0.944), and Backdoor (F1 = 0.907) are all well-classified by at least one model, indicating that the host-disjoint challenge is concentrated on DoS and MitM rather than a uniform degradation.

Fourth, \textbf{the model ranking changes under host-disjoint evaluation}. Under stratified splitting, XGBoost and LightGBM both achieve multiclass F1 of 1.000. Under host-disjoint splitting, Random Forest (macro-F1 = 0.742) outperforms XGBoost (macro-F1 = 0.682), reversing the stratified ranking. Random Forest is particularly strong on Backdoor (F1 = 0.907), while LightGBM achieves the highest Backdoor F1 of 0.926 but fails entirely on MitM (F1 = 0.000).

These host-disjoint results provide a more realistic assessment of operational attack-type classification capability and underscore the importance of split-strategy reporting in IDS evaluation.
...
To assess whether deep learning offers advantages over tabular ensembles for this task, a CNN-LSTM architecture and a Graph Neural Network (GNN) were implemented and evaluated under the behaviour-only regime. An earlier version of the CNN-LSTM suffered from convergence failure (producing constant prediction scores), which was resolved through five architectural fixes: explicit LSTM hidden-state initialisation for MPS compatibility, Kaiming weight initialisation to break output symmetry, unconditional application of \texttt{pos\_weight} to handle majority-class collapse at 76\% prevalence, zero-padding for \texttt{MaxPool1d} on odd-length sequences, and gradient clipping (\texttt{max\_norm=1.0}).

\subsection{CNN-LSTM Detection Performance}
\label{sec:cnnlstm-detection}

Table~\ref{tab:cnnlstm} summarises the CNN-LSTM results on the full TON\_IoT dataset (211,043 rows, behaviour-only, stratified split).
...
The CNN-LSTM achieves PR-AUC $= 0.9998$ and ROC-AUC $= 0.9995$ on the full 211,043-row TON\_IoT dataset, approaching the tree ensembles' perfect 1.000. The binary F1 of 0.9970 and macro-F1 of 0.9936 confirm genuine discriminative learning with well-separated predictions (score range $[0.0003, 0.9999]$). Training required approximately 29 minutes (1,756 seconds) on 147,729 training samples with 862 behaviour-only features.

To assess deployment-realistic generalisation, the CNN-LSTM was additionally evaluated under a host-disjoint split where no source IP address appears in more than one partition (Section~\ref{sec:host-split}). Table~\ref{tab:cnnlstm-host} presents these results.
...
The host-split ROC-AUC of 0.834 represents a substantial drop from the stratified split's 0.9995, confirming that stratified evaluation inflates performance even when identifier features are excluded. Under host-disjoint conditions, the CNN-LSTM must generalise to traffic from previously unseen devices, a requirement that reduces ROC-AUC by 16.5 percentage points. This result provides a more realistic estimate of deployment performance and demonstrates that the CNN-LSTM's convolutional feature extraction partially transfers across hosts, though with measurable degradation.

\subsection{CNN-LSTM XAI Evaluation}
\label{sec:cnnlstm-xai}

Integrated Gradients (IG) attribution was applied to the CNN-LSTM and evaluated against the five-criterion XAI framework. Table~\ref{tab:cnnlstm-xai} presents the results.
...
The CNN-LSTM's IG attributions, when trained on the full 211,043-row dataset, show substantially more concentrated representations than earlier subsample-based results. The $k_{90}$ of 0.014 indicates that only 1.4\% of features account for 90\% of attribution mass, much closer to XGBoost's 0.2\% than the 19.7\% observed on the 5,000-row subsample. The Gini coefficient of 0.988 (vs.\ 0.998 for XGBoost) confirms this convergence: the full-data CNN-LSTM learns to concentrate attribution on a small set of dominant features rather than distributing it broadly. The top-5 IG features---\texttt{dst\_bytes}, \texttt{src\_bytes}, \texttt{src\_pkts}, \texttt{src\_ip\_bytes}, and \texttt{dst\_ip\_bytes}---are plausible volumetric network behaviour indicators, with mean RMA@5 $= 0.40$ across scored attack types. The IG timing of 5.84~ms/sample (computed on a 500-sample subsample) is slower than TreeSHAP (0.04~ms) but remains suitable for near-real-time deployment.

\subsection{GNN Detection Performance}
\label{sec:gnn-results}

The Graph Neural Network (GraphSAGE with edge MLP, Section~\ref{sec:gnn}) was evaluated on the full 211,043-row TON\_IoT dataset using a stratified split. Table~\ref{tab:gnn} presents the results.
...
The GNN achieves PR-AUC $= 0.9997$ and ROC-AUC $= 0.9993$ on the full 211,043-row TON\_IoT dataset, with binary F1 of 0.9973 and macro-F1 of 0.9943. These results are remarkably close to the tree ensemble models, demonstrating that graph structure---where nodes represent IP addresses and edges represent network flows---provides highly effective signal for intrusion detection when sufficient graph density is available. Training required 66 seconds on the full dataset, faster than the CNN-LSTM due to the transductive training regime. The GNN is applicable only to TON\_IoT (which provides source and destination IP columns); CIC-IoT-2023 lacks these columns and is automatically skipped.

\subsection{Cross-Architecture Comparison}
\label{sec:arch-comparison}

Table~\ref{tab:all-models} presents a unified comparison across all model families evaluated on TON\_IoT under the behaviour-only regime.

\begin{table}[H]
\centering
\caption{Cross-architecture comparison on TON\_IoT (behaviour-only regime). OXS is reported for tabular models with full five-criterion evaluation; deep learning models lack full OXS evaluation.}
...
\caption{Binary detection performance across all six model families on TON\_IoT (full dataset, behaviour-only regime, stratified split). Y-axis starts at 0.995 to show differences. All models approach perfect detection; the CNN-LSTM and GNN show marginal but measurable gaps.}
\label{fig:detection}
\end{figure}

On the full 211,043-row dataset, all six model families achieve near-perfect detection (PR-AUC $\geq 0.9997$), confirming that the TON\_IoT detection task is solvable across radically different model architectures. The CNN-LSTM and GNN close the small gap observed on subsampled data, achieving performance within 0.05\% of the tree ensembles. The CNN-LSTM's IG-based explanations, when trained on the full dataset, converge toward the tree models' concentration levels ($k_{90} = 0.014$, Gini $= 0.988$), approaching the tree ensembles' extreme concentration ($k_{90} \leq 0.002$, Gini $\geq 0.998$). The GNN's ability to detect attacks from graph topology validates the inclusion of relational modelling in the HYDRA framework.
...
\caption{SHAP global feature importance for XGBoost (behaviour-only regime). \texttt{conn\_state\_S0} dominates, consistent with its role as the primary indicator of incomplete TCP handshakes characteristic of scanning and DoS attacks.}
...
\caption{OXS radar chart comparing the five XAI evaluation criteria across tabular models (behaviour-only regime). XGBoost (blue) dominates on plausibility and timeliness; LogReg (green) leads on faithfulness; Random Forest (red) shows poor stability.}
\label{fig:oxs-radar}
\end{figure}
...
\caption{Cross-dataset generalisation: source-test vs.\ target ROC-AUC for TON\_IoT $\to$ CIC-IoT-2023. The dashed line at 0.5 marks random-chance performance. LogReg is the only model that generalises above chance; tree ensembles and CNN-LSTM suffer catastrophic ROC-AUC degradation.}
\label{fig:cross-roc}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{CNN-LSTM generalises comparably to LightGBM} (ROC-AUC: 0.985 to 0.285, a drop of 70.0\%). Despite its different architecture, the CNN-LSTM learns similarly dataset-specific patterns on the 6 canonical features. The limited feature space (6 features vs.\ 862 in the within-dataset experiment) constrains the CNN-LSTM's convolutional layers, which require broader feature sequences to extract meaningful local patterns.
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Near-perfect stratified detection masks substantial host-level leakage.} All models achieve PR-AUC and ROC-AUC of 1.000 under stratified splitting, but host-disjoint evaluation reduces XGBoost binary ROC-AUC to 0.842 and the best model's multiclass macro-F1 to 0.742. This 25.8 percentage point multiclass gap demonstrates that published near-perfect results on TON\_IoT are inflated by host-level flow correlation, even when identifier features are excluded.
  \item \textbf{Deep learning matches tree ensembles on the full dataset.} The CNN-LSTM achieves PR-AUC $= 0.9998$ and ROC-AUC $= 0.9995$; the GNN achieves PR-AUC $= 0.9997$ and ROC-AUC $= 0.9993$ on the full 211,043-row dataset. Both approach the tree ensembles' perfect 1.000, demonstrating that the detection task is solvable across all architectures.
  ...
  \item \textbf{Cross-dataset generalisation exposes a critical gap.} Within-dataset performance of 1.000 does not transfer: tree models suffer catastrophic ROC-AUC drops (up to 87\%) on CIC-IoT-2023, the CNN-LSTM drops 70\%, while LogReg loses only 9\%. This underscores the need for generalisation benchmarking in IDS evaluation.
...
The host-disjoint multiclass results (Table~\ref{tab:host-multiclass}) provide the clearest evidence of this inflation. When models must classify attack types on traffic from hosts entirely absent from the training data, macro-F1 drops from 1.000 to 0.742 (Random Forest best)---a 25.8 percentage point decrease. This finding is particularly significant because identifier features were already excluded; the remaining inflation comes from behavioural flow correlation within hosts. The implication for the IDS community is that even ``behaviour-only'' evaluation is insufficient without host-disjoint splitting when the dataset contains host-level structure.
...
Several limitations constrain the scope and generalisability of these findings. The first concerns the deep learning models. The CNN-LSTM achieves PR-AUC $= 0.996$, below the tree models' 1.000 but demonstrating genuine discriminative learning. The remaining performance gap likely reflects the pre-aggregated nature of TON\_IoT's flow features, which lack the sequential structure CNN-LSTM is designed to exploit. Furthermore, the CNN-LSTM and GNN were evaluated on smaller subsamples (5,000 and 3,000 rows respectively) than the full dataset used for tabular models, and their XAI evaluation is less comprehensive than the full five-criterion protocol applied to the tabular models.

The second structural limitation is dataset coverage for the Graph Neural Network. While the GNN achieves PR-AUC $= 0.9997$ on the full TON\_IoT dataset, it requires source and destination IP columns for graph construction, limiting its applicability to datasets that provide these identifiers. CIC-IoT-2023 lacks IP columns and is therefore excluded from GNN evaluation.
...
Both deep learning extensions achieved successful results. The CNN-LSTM, after resolution of five architectural bugs that caused initial convergence failure, achieves PR-AUC $= 0.996$ and ROC-AUC $= 0.990$ with Integrated Gradients attribution producing plausible explanations (Section~\ref{sec:cnnlstm-results}). The Graph Neural Network achieves PR-AUC $= 0.979$ and ROC-AUC $= 0.960$, though evaluation was limited to TON\_IoT due to the requirement for source and destination IP columns.

Table~\ref{tab:obj-achievement} maps each objective from Section~\ref{sec:aim} to its achievement status and supporting evidence.
...
\midrule
O1: Implement model progression (baselines through ensembles) & Achieved & PR-AUC = 1.000 across 4 models, 2 regimes, 3 seeds (Table~\ref{tab:binary}) \\
\addlinespace
O2: Define feature regimes isolating leakage & Achieved & Regime A vs B comparison (Table~\ref{tab:leakage}); host-disjoint split reduces macro-F1 from 1.000 to 0.742 (Table~\ref{tab:host-multiclass}) \\
\addlinespace
O3: Generate per-alert explanation records & Achieved & SHAP, LIME, IG, Anchors implemented; local explanations saved per run \\
\addlinespace
O4: Evaluate explanations against five criteria & Achieved & Tables~\ref{tab:xai-behav}--\ref{tab:xai-ident}; OXS ranking (Table~\ref{tab:oxs}) \\
\addlinespace
O5: Position against concurrent work and generalisation & Achieved & Table~\ref{tab:positioning} (positioning); Table~\ref{tab:cross} (cross-dataset generalisation) \\
\addlinespace
CNN-LSTM extension & Achieved & PR-AUC = 0.996, ROC-AUC = 0.990; IG XAI evaluated (Section~\ref{sec:cnnlstm-results}) \\
\addlinespace
GNN extension & Achieved (TON\_IoT only) & PR-AUC = 0.979, ROC-AUC = 0.960; limited to TON\_IoT (requires IP columns) \\
\bottomrule
\end{tabular}
\end{table}
...
First, the CNN-LSTM convergence issues were resolved through five architectural fixes (Section~\ref{sec:cnnlstm-results}). Further work should explore attention-based tabular models such as TabNet and FT-Transformer, which may be better suited to pre-aggregated flow features and could close the remaining performance gap between deep learning and tree ensemble models.
...
Apache Software Foundation, 2024. \textit{Apache Kafka documentation} [online]. Available from: \url{https://kafka.apache.org/} [Accessed 10 February 2026].
...
Bhatt, P., Moradan, A., Parekh, M. and Patel, V., 2025. Explainable AI-driven intrusion detection in IoT: a comparative analysis of SHAP and LIME. \textit{Frontiers in Computer Science}, 7, article 1509631. Available from: \url{https://doi.org/10.3389/fcomp.2025.1509631} [Accessed 15 March 2026].
...
Dang, Y., Lin, Q., Huang, P., Si, D., Zhang, D. and Lyu, M.R., 2023. AIOps: real-world challenges and research innovations. \textit{IEEE Software}, 40(3), pp.45--52. Available from: \url{https://doi.org/10.1109/MS.2019.2937000} [Accessed 15 March 2026].
...
Fang, P., Gao, P., Liu, C., Ayday, E., Jee, K., Wang, T., Ye, Y., Liu, Z. and Xiao, X., 2022. Back-propagating system dependency impact for attack investigation. In: \textit{31st USENIX Security Symposium (USENIX Security 2022)}, pp.3091--3108.

\bibitem[Ferrag et~al., 2022]{ferrag2022}
Ferrag, M.A., Friha, O., Hamouda, D., Maglaras, L. and Janicke, H., 2022. Analysis of ToN-IoT, UNW-NB15, and Edge-IIoT datasets using deep learning in cybersecurity for IoT. \textit{Applied Sciences}, 12(19), article 9572. Available from: \url{https://doi.org/10.3390/app12199572} [Accessed 15 March 2026].
...
Islam, R., Sayed, M.I., Saha, S., Hossain, M.J. and Masud, M.A., 2024. E-XAI: evaluating black-box explainable AI frameworks for network intrusion detection. \textit{IEEE Access}, 12, pp.19859--19884. Available from: \url{https://doi.org/10.1109/ACCESS.2024.3362873} [Accessed 15 March 2026].

\bibitem[Ke et~al., 2017]{ke2017}
Ke, G., Meng, Q., Finley, T., Wang, T., Chen, W., Ma, W., Ye, Q. and Liu, T.-Y., 2017. LightGBM: a highly efficient gradient boosting decision tree. In: \textit{Advances in Neural Information Processing Systems 30 (NeurIPS 2017)}, pp.3146--3154.
...
Lanvin, M., Giura, P., Mallouli, W. and de Oca, E.M., 2022. Errors in the CICIDS2017 dataset and the significant differences in detection performances it makes. In: \textit{Proceedings of the 17th International Conference on Risks and Security of Internet and Systems (CRiSIS 2022)}. Springer, LNCS.
...
Lo, W.W., Layeghy, S., Sarhan, M., Gallagher, M. and Portmann, M., 2023. Hermes: lateral movement detection via heterogeneous graph attention networks. In: \textit{Proceedings of the 2023 ACM SIGSAC Conference on Computer and Communications Security (CCS '23)}.
...
Lundberg, S.M., 2022. \textit{SHAP: a unified approach to explaining the output of any machine learning model} [Software, v0.42]. Available from: \url{https://github.com/shap/shap} [Accessed 15 March 2026].

\bibitem[Malhotra et~al., 2016]{malhotra2016}
Malhotra, P., Ramakrishnan, A., Anand, G., Vig, L., Agarwal, P. and Shroff, G., 2016. LSTM-based encoder-decoder for multi-sensor anomaly detection. In: \textit{Proceedings of the ICML 2016 Anomaly Detection Workshop}.

\bibitem[Mart\'{i}n et~al., 2022]{martin2022}
Mart\'{i}n, C., Langendoerfer, P., Soltani Zarrin, P., D\'{i}az, M. and Rubio, B., 2022. Kafka-ML: connecting the data stream with ML/AI frameworks. \textit{Future Generation Computer Systems}, 145, pp.1--14. Available from: \url{https://doi.org/10.1016/j.future.2021.07.037} [Accessed 15 March 2026].
...
Munkhdorj, T., Amin, M.B., Batbaatar, E. and Lee, S., 2017. Cyber attack prediction using social data analysis. In: \textit{2017 IEEE 7th Annual Computing and Communication Workshop and Conference (CCWC)}. IEEE.

\bibitem[Nassif et~al., 2021]{nassif2021}
Nassif, A.B., Talib, M.A., Nasir, Q. and Dakalbab, F.M., 2021. Machine learning for anomaly detection: a systematic review. \textit{IEEE Access}, 9, pp.78658--78700.
...
Ogunseyi, T.B., Thiyagarajan, G., He, H., Bist, V. and Du, Z., 2026. Performance analysis of explainable deep learning-based intrusion detection systems for IoT networks: a systematic review. \textit{Applied Sciences}. Available from: \url{https://www.mdpi.com/1424-8220/26/2/363} [Accessed 20 March 2026].

\bibitem[Pansare et~al., 2024]{pansare2024}
Pansare, S., Malik, A. and Batra, I., 2024. Hybrid machine learning algorithm for intrusion detection systems. \textit{IEEE Access}, 12, pp.74391--74407. Available from: \url{https://doi.org/10.1109/ACCESS.2024.3406053} [Accessed 15 March 2026].
...
Pedregosa, F., Varoquaux, G., Gramfort, A., Michel, V., Thirion, B., Grisel, O., Blondel, M., Prettenhofer, P., Weiss, R., Dubourg, V., Vanderplas, J., Passos, A., Cournapeau, D., Brucher, M., Perrot, M. and Duchesnay, E., 2011. Scikit-learn: machine learning in Python. \textit{Journal of Machine Learning Research}, 12, pp.2825--2830.

\bibitem[Popoola et~al., 2021]{popoola2021}
Popoola, S.I., Ande, R., Adebisi, B., Gui, G., Hammoudeh, M. and Jogunola, O., 2021. Federated deep learning for zero-day botnet attack detection in IoT-edge devices. \textit{IEEE Internet of Things Journal}, 9(5), pp.3930--3944. Available from: \url{https://doi.org/10.1109/JIOT.2021.3105197} [Accessed 15 March 2026].

\bibitem[Qi et~al., 2023]{qi2023}
Qi, J., Luan, T., Zhao, J. and Chen, S., 2023. LogFiT: log anomaly detection using fine-tuned language models. \textit{IEEE Transactions on Knowledge and Data Engineering}, 36(5), pp.2290--2302.

\bibitem[Raeiszadeh et~al., 2025]{raeiszadeh2025}
Raeiszadeh, M., Estrada-Solano, F., Glitho, R.H., Eker, J. and Mini, R.A.F., 2025. ALogSCAN: a self-supervised dual network for adaptive and timely log anomaly detection in clouds. \textit{IEEE Transactions on Network and Service Management}, 22(1), pp.1105--1119. Available from: \url{https://doi.org/10.1109/TNSM.2024.3511554} [Accessed 15 March 2026].

\bibitem[Rani et~al., 2024]{rani2024}
Rani, B.P., Lahari, C.S., Lavanya, J.A., Lakshmanarao, A. and Kosuri, G.V., 2024. Advanced intrusion detection system through hybrid integration of random forest and deep learning with artificial neural networks. \textit{IEEE Access}, 12, pp.182661--182675. Available from: \url{https://doi.org/10.1109/ACCESS.2024.3512553} [Accessed 15 March 2026].
...
Robinson, I., Webber, J. and Eifrem, E., 2015. \textit{Graph Databases: New Opportunities for Connected Data}. 2nd ed. Sebastopol, CA: O'Reilly Media.
...
Schwarzschild, A., Goldblum, M., Gupta, A., Dickerson, J.P. and Goldstein, T., 2021. Just how toxic is data poisoning? A unified benchmark for backdoor and data poisoning attacks. In: \textit{Proceedings of the 38th International Conference on Machine Learning (ICML 2021)}, pp.9382--9392.
...
S\"{o}nmez, Y. and \"{O}zdemir, S., 2023. Predictive intrusion detection: a survey of methods, metrics, and datasets. \textit{IEEE Access}, 11, pp.86254--86278. Available from: \url{https://doi.org/10.1109/ACCESS.2023.3304008} [Accessed 15 March 2026].

\bibitem[Srivastava et~al., 2024]{srivastava2024}
Srivastava, G., Jhaveri, R.H., Bhatt, S., Agrawal, S. and Raj, R., 2024. An explainable AI framework for intrusion detection systems. \textit{Applied Sciences}, 14(2), article 753. Available from: \url{https://doi.org/10.3390/app14020753} [Accessed 15 March 2026].
...
Viirlaid, A., Shalaginov, A., Bhatia, S. and Gurina, R., 2025. Evaluation of explainable AI approaches for network intrusion detection systems in security operations contexts. \textit{Baltic Journal of Modern Computing}, 13(2), pp.183--201.

\bibitem[Ying et~al., 2019]{ying2019}
Ying, Z., Bourgeois, D., You, J., Zitnik, M. and Leskovec, J., 2019. GNNExplainer: generating explanations for graph neural networks. In: \textit{Advances in Neural Information Processing Systems}, 32. Available from: \url{https://arxiv.org/abs/1903.03894} [Accessed 15 March 2026].

\bibitem[Zhu et~al., 2023]{zhu2023}
Zhu, J., He, S., He, P., Liu, J., Zhang, C. and Lyu, M.R., 2023. Loghub: a large collection of system log datasets for AI-driven log analytics. In: \textit{IEEE International Symposium on Software Reliability Engineering (ISSRE)}. IEEE.
